...
Current settings are `references/policy.json`; recommended reset values are `references/default-policy.json`. For precise model/effort boundaries, examples, fallback order and official sources, read [task boundaries](task-boundaries.md). Those recommendations do not prove model quality or subscription savings. A console restore loads the entire recommended policy as a draft, then requires an explicit apply. Keep existing model choices unless the user requests new defaults or accepts the reset.
