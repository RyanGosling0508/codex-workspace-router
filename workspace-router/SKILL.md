...
The recommended `evidence-v1` classifier requires `task.assessment` with kind, specification, verification, scope, input_form, boundary and concrete evidence. It applies the highest matching rule and preserves the declared complexity as a floor. Text-only, exact, mechanically checkable, local extraction/transformation with low consequence and low uncertainty uses Luna/high. Bounded routine work uses Sol/medium; cross-component, open, uncertain or judgment-heavy investigation uses Sol/high; high-consequence work and open/uncertain system design use Astra/high. Missing assessment means no automatic dispatch. These are operational boundaries, not guarantees of model quality.

Read [presets and evidence](references/presets.md) for trial and premium ambiguity rules. Economy and Balanced may try Luna/high for an exact, local, low-risk, low-uncertainty text-only research question with no writes, `verification: sources` and concrete `source_evidence` naming bounded sources and the fact-check method. Conflicting sources, inferred correctness and open research do not qualify. Economy also permits strictly testable low-risk local implementation trials. Both trials retain the routine recovery floor. Premium promotes evidenced adjacent-lane ambiguity. Record `boundary_evidence` when boundary is adjacent; significant unresolved uncertainty remains high. Every preset preserves acceptance and the direct-by-default gate.
